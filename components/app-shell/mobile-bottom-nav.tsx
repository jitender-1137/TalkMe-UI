...
export function MobileBottomNav() {
  ...
  const isInChatView = activeTab === "chats" && !showMobileSecondaryPanel
  const isInMatchChatView = activeTab === "match" && (matchStatus === "matched" || matchStatus === "disconnected")
  const isInLobbyChatView = activeTab === "match" && lobbySelectedUser !== null

  if (isInChatView || isInMatchChatView || isInLobbyChatView) {
    return null
  }

  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden safe-area-bottom">
      <div className="mx-2 mb-2 rounded-2xl bg-card border border-white/10 shadow-lg shadow-black/5">
